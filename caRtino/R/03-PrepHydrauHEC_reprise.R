...
#' @noRd 
HEC_RAS_Project = function(rep_modele_reprise, name)                                       
  # LPo : création du fichier de projet HECRAS (.prj) + écriture
{  
  template = file.path(rep_modele_reprise, "template.prj")
  x = readLines(template)
  y = gsub("Proj Title=", paste0("Proj Title=", name), x )
  cat(y, file=file.path(rep_modele_reprise, paste0(name, ".prj")), sep="\n")
}

#' Write hec-ras plan                                                           
#' # LPo : Création du fichier plan HECRAS (.p01) + écriture
#' @noRd 
HEC_RAS_Plan = function(rep_modele_reprise, name)
{
  template = file.path(rep_modele_reprise, "template.p01")
  x = readLines(template)
  y = gsub("Plan Title=", paste0("Plan Title=", name), x )
  cat(y, file=file.path(rep_modele_reprise, paste0(name, ".p01")), sep="\n")
}
...
HEC_RAS_Flow = function(Bief_Q, slope_val, name, Profil_name,
                        Reach_name, rep_modele_reprise)
{
  Flow_name = file.path(rep_modele_reprise, paste0(name, ".f01"))
  River_name = name
  write(paste("Flow Title=", name, "\n",
              paste0("Program Version=", version_hecras_reprise),                  # LPo : version d'HECRAS utilisée
  ...
  {
    write(paste("River Rch & RM=", name, ",", Reach_name,
                "               ,", nQ-k+1, "\n",
                formatC(Bief_Q[k], digits=3, width=8, format="f"), sep=""),
...
#' @noRd 
Hec_RAS_Geom_Part1 = function(df_bief, rep_modele_reprise, name_run)
{
  geom_name = file.path(rep_modele_reprise, paste0(name_run,".g01"))
  write(paste("Geom Title=", name_run, "\n",
              paste0("Program Version=", version_hecras_reprise), sep=""),
        file=geom_name)
  Reach_name = "Channel"
...
#run_hydrau_hecras_reprise = function(df_bief, rep_res, name_run, Bief_MNT_Iter, Bief_STA)           

run_hydrau_hecras_reprise = function(df_bief_reprise, rep_modele_reprise, rep_calcul_reprise,
                                     name_reach_reprise, Bief_MNT_reprise, Bief_Q_reprise, coeff)
{
  ## some parameters and constant values for hec-ras
  Profil_name = hecras_profil_name
  Reach_name = hecras_reach_name
  Manning = coeff
  #Manning = hecras_manning_reprise[i_hecras_reprise]
  slope_val = hecras_slope_val
  
  ## create some input files
  #dir_prj = file.path(rep_resu, rep_hecras); dir.create(dir_prj, recursive=T)
  HEC_RAS_Project(rep_modele_reprise, name_reach_reprise)
  HEC_RAS_Plan(rep_modele_reprise, name_reach_reprise)
  
  ## write first part of the geometry file
  Begining = Hec_RAS_Geom_Part1(df_bief_reprise, rep_modele_reprise, name_reach_reprise)

  ## loop over the sections
  Nprofils = nrow(Bief_Q_reprise)
  for(j in 1:Nprofils)
  {
    Profil_MNT = Bief_MNT_reprise %>% filter(NSection==j)
    HEC_RAS_Geom_Part2(Begining, Nprofils-j+1, Profil_MNT, df_bief_reprise$ECART[j], Manning)
  }
  ...
  ## write flow file
  HEC_RAS_Flow(Bief_Q_reprise$Q_m3_s, slope_val, name_reach_reprise, Profil_name, Reach_name, rep_modele_reprise)
  
  
  
  ## run and get results with c++ exe and api                                               
  # LPo : résultats avec c++ exe et api                   
  nomfichier = file.path(rep_modele_reprise, name_reach_reprise)
  #nomfichier = file.path(getwd(),rep_modele_reprise, name_reach_reprise)
  txtfile = file.path(rep_modele_reprise, paste0("nomfichier.txt"))
  write(nomfichier, txtfile)
  file.copy(from=file.path(dir_exe, exe_hecras), to=rep_modele_reprise)
  tmp_wd = getwd(); setwd(rep_modele_reprise)
  system(exe_hecras, show.output.on.console=T); setwd(tmp_wd)
  

    csvfile = file.path(rep_modele_reprise, paste0(name_reach_reprise, ".csv"))
  # file.path(getwd(), rep_modele_reprise
  #           , paste0(name_reach_reprise, ".csv"))
  ...
  Bief_results_final %<>% mutate_all(as.numeric)
  Bief_results_final$NSection = Bief_Q_reprise$NSection
  Bief_results_final$NBief = Bief_Q_reprise$NBief
  Bief_results_final$NBasVers = Bief_Q_reprise$NBasVers
  
  #LPO : Rajout de la colonne Z_m
  Bief_results_final$Z_m = (Bief_results_final$SL_m - Bief_results_final$Hydraulic_Depth_m)
